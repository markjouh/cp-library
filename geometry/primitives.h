...
// Geometry primitives. Sticks to integer arithmetic when possible - everywhere but point to line & line intersection ops.
...
bool safe_eq(T a, T b = 0) {
    ...
    if constexpr (is_floating_point_v<T>) return abs(a - b) < EPS;
    return a == b;
}
...
struct Point {
    ...
    template <class U>
    operator Point<U>() const { return Point<U>(static_cast<U>(x), static_cast<U>(y)); }
    ...
    bool operator<(const Point &b) const { return this->x < b.x || (this->x == b.x && this->y < b.y); }
    bool operator>(const Point &b) const { return b < *this; }
    bool operator==(const Point &b) const { return this->x == b.x && this->y == b.y; }
    bool operator!=(const Point &b) const { return !(*this == b); }

    int side() const { return y < 0 ? -1 : y == 0 && x >= 0 ? 0 : 1; }
    friend bool comp_angle(const Point &a, const Point &b) { return a.side() < b.side() || (a.side() == b.side() && (a ^ b) > 0); }

    friend prod_t<T> mag_sq(const Point &a) { return a * a; }
    friend ld mag(const Point &a) { return sqrt(mag_sq(a)); }
    friend prod_t<T> dist_sq(const Point &a, const Point &b) { return mag_sq(a - b); }
    friend ld dist(const Point &a, const Point &b) { return mag(a - b); }
...
struct Line {
    ...
    bool operator<(const Line &b) const { return comp_angle(this->d, b.d); }
    bool operator>(const Line &b) const { return b < *this; }

    friend ostream &operator<<(ostream &os, const Line &a) { return os << a.p << " + t" << a.d; }
};
...
ld dist_seg(const Point<T> &p, const Line<T> &s) {
    ...
    if ((p - s.p - s.d) * s.d >= 0) return dist(p, s.p + s.d);
