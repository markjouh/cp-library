...
LST(){}
LST(int n,T v=e()):LST(vector(n,v)){}
LST(vector<T>a):n(size(a)),L(__lg(n-1)+1),S(1<<L),s(2*S,e()),z(S,ie()){copy(all(a),&s[S]);for(int i=S;--i;)pl(i);}
void set(int l,T v){l+=S;int r=l+1;PD s[l]=v;PU}
